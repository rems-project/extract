...
let gen_herdtools_ast_to_shallow_ast map i = 
  if i.mnemonics = [] then failwith i.name;
  let m = List.hd i.mnemonics in (* hoping those are same, checked elsewhere *)
      ...
      (fun idx f -> match f with
        | (BinRep.Ifield (n,(_,1))) ->
            ...
            let conv_fld =
              ...
              | "setlk" -> Printf.sprintf "(trans_lk %s)" fld
              | _ -> fld in
            Printf.sprintf "int_to_bit %s" conv_fld
        | (BinRep.Ifield (n,(_,sz))) ->
            let fld = 
              try List.nth fps (find_param_num idx) 
              with Not_found -> 
            ...
            let ifield = List.nth i.ifields idx in
            if is_imm ifield map 
            then Printf.sprintf "Sail_values.to_vec0 true (Nat_big_num.of_int %d,Nat_big_num.of_int %s)" 
                sz fld
                (* for registers *)
            else Printf.sprintf "Sail_values.to_vec0 true (Nat_big_num.of_int 5,Nat_big_num.of_int (int_of_reg %s))"
                fld
        | (BinRep.SplitIfield (n,(_,sz1),(_,sz2))) ->
            let fld = 
              try List.nth fps (find_param_num idx) 
            ...
            in
            let sz = sz1 + sz2 in
            Printf.sprintf "Sail_values.to_vec0 true (Nat_big_num.of_int %d,Nat_big_num.of_int %s)" 
                sz fld
...
let gen_shallow_ast_to_herdtools_ast map i = 
  if i.mnemonics = [] then failwith i.name;
  let m = List.hd i.mnemonics in (* hoping those are same, checked elsewhere *)
         ...
         | (BinRep.Ifield (n,(_,1))) ->
           ...
           let ((tmi,mi),use_tmi) = 
            ...
            let conv_fld =
              ...
              | "setaa" -> Printf.sprintf "(trans_out_aa %s)" (if use_tmi then tmi else n)
              | "setlk" -> Printf.sprintf "(trans_out_lk %s)" (if use_tmi then tmi else n)
              | _ -> Printf.sprintf "(trans_out_bit %s)" (if use_tmi then tmi else n) in
            (n,(mi,Printf.sprintf "%s" conv_fld))
         | (BinRep.Ifield (n,(_,sz))) ->
           let n = ocamlify_names n in
            let ((tmi,mi),use_tmi) = 
